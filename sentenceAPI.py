...
# Ensure you have a Config.py file with HF_API_KEY defined in the same directory.
...
DEMOS = [
    ...
    ("nearest hospital near to me", "closest clinic near me"),
    ("mobile games are getting bigger in size", "game size on mobile phone is increasing"),
    ("is it going to rain today", "today it's rainy"),
...
def show_results(s):
    print(f"\n 🎯 Results of Similarity: {round(s * 100, 1)}% [{bar(s)}] -> {label(s)}")
...
def show_flow(q1, q2):
    ...
    print("\n FLOW (sentence 0 -> strongest/helper/least -> similarity %)")
    print("\n1) Input sentence")
    print(f"  Q1: {q1}\n  Q2: {q2}")
    
    print("\n2) Split into word/tokens (same as you typed)")
    print(f"  Q1 -> {TOK(q1)}\n  Q2 -> {TOK(q2)}")
...
def run(q1, q2, title):
    print(f"\n--- {title} ---")
    base = hf(q1, q2)
    strong = sorted({w for w in clean(q1) if len(w) >= 4})
