    ...
    if st.button("Ask",key="a_ata"):
        if not q.strip(): st.warning("Enter a question")
            ...
            with st.spinner("Thinking..."):
                st.session_state.history_ata.append({"question":q.strip(),"answer":teachinganser(q.strip())})
            st.rerun()
...
def run_mathmastermind():
    ...
    with st.form("mm_form",clear_on_submit=True):
        q = st.text_area("Maths problem",height=100,key=f"mm_{st.session_state.k_mm}")
        a,b = st.columns([3,1])
