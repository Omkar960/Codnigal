...
def spinner():
    ...
    while not stop_event.is_set():
        sys.stdout.write("\rRecording..."+ spinnerchars[idx%len(spinnerchars)])
        sys.stdout.flush()
        idx += 1
        time.sleep(0.1)
    sys.stdout.write("\rRecording stopped.\n")
def recorduntilenter():
    p = pyaudio.PyAudio()
...
def saveaudio(data,rate,width,filename="my_audio.wav"):
...
def show_waveform(data,rate):
    ...
    plt.show()
def main():
    print("???? Speak into mic. Press Enter to stop.")
    audio,rate,width = recorduntilenter()
    saveaudio(audio,rate,width)
    transcribeaudio(audio,rate,width)
